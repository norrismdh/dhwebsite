// =============================================================
// Digital Hive — nav + footer generator (SINGLE SOURCE OF TRUTH)
//
// The nav and footer are baked into every page as static, crawlable
// HTML (root-absolute links, so the markup is byte-identical on every
// page regardless of folder depth). scripts/site.js only *enhances*
// this markup (mega-menu open/close, mobile toggle, active-state).
//
// Edit the IA below, then run:  npm run nav
// It rewrites the <header class="nav"> and <footer class="footer">
// blocks in every page. No Vercel build step — the output is committed.
// =============================================================

import { readFileSync, writeFileSync, readdirSync, statSync } from 'node:fs';
import { join, dirname } from 'node:path';
import { fileURLToPath } from 'node:url';

const ROOT = join(dirname(fileURLToPath(import.meta.url)), '..');

// Root-absolute link: leave external / hash / already-absolute alone.
const abs = (h) => /^(https?:|mailto:|#|\/)/.test(h) ? h : '/' + h;

// ---------- Icons (hex-framed line glyphs) ----------
const ICO = {
  grid:      `<rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/>`,
  plug:      `<path d="M9 2v6"/><path d="M15 2v6"/><rect x="7" y="8" width="10" height="6" rx="1"/><path d="M12 14v4a3 3 0 0 0 3 3"/>`,
  layers:    `<polygon points="12 2 22 8 12 14 2 8 12 2"/><polyline points="2 16 12 22 22 16"/>`,
  blueprint: `<rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M9 3v18"/>`,
  question:  `<circle cx="12" cy="12" r="10"/><path d="M9.1 9a3 3 0 0 1 5.8 1c0 2-3 3-3 3"/><line x1="12" y1="17" x2="12" y2="17.5"/>`,
  book:      `<path d="M4 4h10a4 4 0 0 1 4 4v12H8a4 4 0 0 1-4-4z"/><path d="M4 16a4 4 0 0 1 4-4h10"/>`,
  users:     `<circle cx="9" cy="8" r="3.5"/><circle cx="17" cy="9" r="2.5"/><path d="M2.5 20a6.5 6.5 0 0 1 13 0"/><path d="M15 20a5 5 0 0 1 6.5-4.8"/>`,
  pencil:    `<path d="M4 20h4l11-11-4-4L4 16z"/><path d="M14 6l4 4"/>`,
  shield:    `<path d="M12 3l8 4v5c0 5-3.5 8.5-8 9-4.5-.5-8-4-8-9V7z"/><polyline points="9 12 11 14 15 10"/>`,
  info:      `<circle cx="12" cy="12" r="10"/><path d="M12 16v-5"/><circle cx="12" cy="8" r=".7" fill="currentColor"/>`,
  handshake: `<path d="M2 12l4-4 4 4 4-4 4 4 4-4"/><path d="M6 12l3 4 3-3 3 3 3-4"/>`,
  mail:      `<rect x="3" y="5" width="18" height="14" rx="2"/><polyline points="3 7 12 13 21 7"/>`,
};

const hexIcon = (paths) => `
      <span class="mega__ico" aria-hidden="true">
        <svg class="mega__hex" viewBox="0 0 32 32"><polygon points="16,1.5 28.5,9 28.5,23 16,30.5 3.5,23 3.5,9" fill="none" stroke="currentColor" stroke-width="1.4"/></svg>
        <svg class="mega__glyph" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round">${paths}</svg>
      </span>`;

// ---------- Information architecture — the three mega panels ----------
const MEGA = {
  platform: {
    label: 'Platform',
    href: 'Product',
    eyebrow: 'The Digital Hive Platform',
    title: 'A unified analytics experience above your existing BI stack.',
    items: [
      { ico: ICO.grid,      label: 'Platform overview', sub: 'Catalog, Analytics Hub, governance',   href: 'Product' },
      { ico: ICO.plug,      label: 'Connectors',         sub: 'BI, AI, data & document tools',        href: 'Connectors' },
      { ico: ICO.layers,    label: 'Use cases',          sub: 'By situation and industry',            href: 'UseCases' },
      { ico: ICO.blueprint, label: 'Architecture',       sub: 'Deployment and scale',                 href: 'Architecture' },
      { ico: ICO.shield,    label: 'Security',           sub: 'Certifications, compliance, trust',    href: 'Security' },
    ],
    cta: { label: 'See the full platform', href: 'Product' },
  },
  resources: {
    label: 'Resources',
    href: 'Resources',
    eyebrow: 'Learn',
    title: 'Independent research, working definitions, honest comparisons.',
    items: [
      { ico: ICO.question, label: 'FAQ',              sub: 'Common questions answered', href: 'FAQ' },
      { ico: ICO.book,     label: 'Resource library', sub: 'Reports, guides, briefs',   href: 'Resources' },
      { ico: ICO.users,    label: 'Customer stories', sub: 'Real outcomes, real teams', href: 'Customers' },
      { ico: ICO.pencil,   label: 'Blog',             sub: 'Field notes from the team', href: 'Blog' },
      { ico: ICO.shield,   label: 'Trust Center',     sub: 'Security & compliance',     href: 'https://trust.digitalhive.com/' },
    ],
    cta: { label: 'Open the library', href: 'Resources' },
  },
  company: {
    label: 'Company',
    href: 'About',
    eyebrow: 'Company',
    title: 'The category-defining analytics hub for complex enterprises.',
    items: [
      { ico: ICO.info,      label: 'About Digital Hive', sub: 'Why we exist',         href: 'About' },
      { ico: ICO.users,     label: 'Leadership',         sub: 'The people behind it', href: 'About#team' },
      { ico: ICO.handshake, label: 'Partners',           sub: 'Resellers & ISVs',     href: 'Partners' },
      { ico: ICO.mail,      label: 'Contact us',         sub: 'Get in touch',         href: 'Contact' },
    ],
    cta: { label: 'Meet the team', href: 'About#team' },
  },
};

const buildTrigger = (key, def) => {
  const itemsHTML = def.items.map(it => `
        <a class="mega__item" href="${abs(it.href)}">
          ${hexIcon(it.ico)}
          <span class="mega__txt">
            <span class="mega__label">${it.label}</span>
            <span class="mega__sub">${it.sub}</span>
          </span>
        </a>`).join('');

  return `
        <li class="nav__item" data-mega="${key}">
          <a class="nav__trigger" href="${abs(def.href)}" aria-haspopup="true" aria-expanded="false">
            <span>${def.label}</span>
            <svg class="nav__chev" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg>
          </a>
          <div class="mega__panel" role="menu" aria-hidden="true" inert>
            <span class="mega__caret" aria-hidden="true"></span>
            <div class="mega__panel-inner">
              <div class="mega__lede">
                <span class="eyebrow">${def.eyebrow}</span>
                <p>${def.title}</p>
                <a class="mega__cta" href="${abs(def.cta.href)}">
                  ${def.cta.label}
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>
                </a>
              </div>
              <div class="mega__items">${itemsHTML}</div>
            </div>
          </div>
        </li>`;
};

const mobileGroup = (def) => `
        <section class="mobile-menu__group">
          <h5>${def.label}</h5>
          <ul>
            ${def.items.map(it => `<li><a href="${abs(it.href)}">${it.label}</a></li>`).join('')}
          </ul>
        </section>`;

// ---------- Nav inner (goes inside <header class="nav">) ----------
const NAV = `
      <div class="container nav__inner">
        <a class="nav__logo" href="/" aria-label="Digital Hive home">
          <img src="/assets/logo-colour.svg" alt="Digital Hive" width="349" height="102" />
        </a>
        <nav aria-label="Primary">
          <ul class="nav__links">${
            buildTrigger('platform', MEGA.platform) +
            buildTrigger('resources', MEGA.resources) +
            buildTrigger('company', MEGA.company) +
            `
        <li class="nav__item"><a class="nav__plain" href="/Pricing">Pricing</a></li>`
          }</ul>
        </nav>
        <div class="nav__cta">
          <a class="btn btn--primary" href="/Demo">Book a demo</a>
          <button type="button" class="nav__toggle" data-mobile-toggle aria-label="Open menu" aria-expanded="false">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><line x1="4" y1="7" x2="20" y2="7"/><line x1="4" y1="12" x2="20" y2="12"/><line x1="4" y1="17" x2="20" y2="17"/></svg>
          </button>
        </div>
      </div>
      <div class="container mobile-menu" data-mobile-menu>${
        mobileGroup(MEGA.platform) +
        mobileGroup(MEGA.resources) +
        mobileGroup(MEGA.company) +
        `
        <section class="mobile-menu__group">
          <ul><li><a href="/Pricing">Pricing</a></li></ul>
        </section>`
      }
      </div>
    `;

// ---------- Footer inner (goes inside <footer class="footer">) ----------
const FOOTER = `
      <div class="container">
        <div class="footer__top">
          <div class="footer__brand">
            <img src="/assets/logo-reversed.svg" alt="Digital Hive" width="349" height="102" loading="lazy" decoding="async" />
            <p>Where analytics finally make sense. An analytics hub.</p>
            <div class="footer__social">
              <a href="https://www.linkedin.com/company/digital-hive-bi-portal" aria-label="LinkedIn" target="_blank" rel="noopener"><svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M4.98 3.5C4.98 4.88 3.87 6 2.5 6S0 4.88 0 3.5 1.12 1 2.5 1s2.48 1.12 2.48 2.5zM0 8h5v16H0V8zm7.5 0H12v2.2h.07c.63-1.2 2.18-2.46 4.49-2.46 4.8 0 5.69 3.16 5.69 7.27V24h-5v-7.2c0-1.72-.03-3.93-2.4-3.93-2.4 0-2.77 1.87-2.77 3.81V24h-5V8z"/></svg></a>
              <a href="https://www.youtube.com/@digitalhive4376" aria-label="YouTube" target="_blank" rel="noopener"><svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M23.5 6.2a3 3 0 0 0-2.1-2.1C19.5 3.5 12 3.5 12 3.5s-7.5 0-9.4.6A3 3 0 0 0 .5 6.2C0 8.1 0 12 0 12s0 3.9.5 5.8a3 3 0 0 0 2.1 2.1c1.9.6 9.4.6 9.4.6s7.5 0 9.4-.6a3 3 0 0 0 2.1-2.1c.5-1.9.5-5.8.5-5.8s0-3.9-.5-5.8zM9.6 15.6V8.4l6.3 3.6-6.3 3.6z"/></svg></a>
            </div>
          </div>
          <div class="footer__col"><h3>Product</h3><ul>
            <li><a href="/Product#catalog">Catalog</a></li>
            <li><a href="/Product#capabilities">Analytics Hub</a></li>
            <li><a href="/Product#governance">Governance</a></li>
            <li><a href="/Connectors">Connectors</a></li>
          </ul></div>
          <div class="footer__col"><h3>Resources</h3><ul>
            <li><a href="/UseCases">Use cases</a></li>
            <li><a href="/Customers">Customers</a></li>
            <li><a href="/Pricing">Pricing</a></li>
            <li><a href="https://support.digitalhive.com">Support</a></li>
          </ul></div>
          <div class="footer__col"><h3>Company</h3><ul>
            <li><a href="/About">About</a></li>
            <li><a href="/Blog">Blog</a></li>
            <li><a href="/FAQ">FAQ</a></li>
            <li><a href="/Contact">Contact</a></li>
          </ul></div>
        </div>
        <div class="footer__bottom">
          <span>&copy; 2026 Digital Hive</span>
          <ul>
            <li><a href="/Privacy">Privacy</a></li>
            <li><a href="/Terms">Terms</a></li>
            <li><a href="https://trust.digitalhive.com/" target="_blank" rel="noopener">Trust center</a></li>
          </ul>
        </div>
      </div>
    `;

// ---------- Injection ----------
const NAV_RE = /(<header class="nav"[^>]*>)[\s\S]*?(<\/header>)/;
const FOOTER_RE = /(<footer class="footer"[^>]*>)[\s\S]*?(<\/footer>)/;

function collectHtml(dir, out = []) {
  for (const name of readdirSync(dir)) {
    if (name === 'node_modules' || name === 'dhadmin' || name === 'ops' || name === '.git') continue;
    const full = join(dir, name);
    const st = statSync(full);
    if (st.isDirectory()) collectHtml(full, out);
    else if (name.endsWith('.html')) out.push(full);
  }
  return out;
}

let navCount = 0, footerCount = 0, skipped = [];
for (const file of collectHtml(ROOT)) {
  let html = readFileSync(file, 'utf8');
  const before = html;
  if (NAV_RE.test(html))    { html = html.replace(NAV_RE, `$1${NAV}$2`); navCount++; }
  if (FOOTER_RE.test(html)) { html = html.replace(FOOTER_RE, `$1${FOOTER}$2`); footerCount++; }
  if (html !== before) writeFileSync(file, html);
  else if (!/<header class="nav"/.test(before)) skipped.push(file);
}

console.log(`nav injected into ${navCount} pages, footer into ${footerCount} pages`);
